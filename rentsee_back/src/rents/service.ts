...
export async function find(query = {}) {
  return db.rents.find(query).sort({created:-1}).toArray();
}
export function findOne(query) {
  return db.rents.findOne(query);
}
export function findById(id: string) {
  return db.rents.aggregate([
...
export function patch(query: any, rent: any) {
  return db.rents.findOneAndUpdate(query, { $set: rent }, { upsert: false, returnOriginal: false });
}
